...
class DBConnection {
    ...
    async GetPG(table, where = "", values = [], select = "*", output = false) {
        ...
        return new Promise(function (resolve, reject) {
            if (typeof where != "string") {
                reject("Error GetPG: Input is not a String, type: " + typeof where + ".");
            }
            let query;
            if (where == "") {
                query = {
            ...
            thisObject.Database.query(query, (err, res) => {
                if (err) {
                    ...
                    if (output) {
                        thisObject.Database.query({ text: `select * from information_schema.columns where table_name = '${table}'`, values: [] }, (err, columns) => {
                            if (err) {
                                reject(err)
                            } else {
                                let Keys = {};
                                for (let i = 0; i < columns.rows.length; i++) {
                                    Keys[columns.rows[i].column_name] = columns.rows[i].data_type;
                                }
                                res.rows = res.rows.map(function (data) {
                                    ...
                                    for (let i = 0; i < columns.rows.length; i++) {
                                        if (Keys[columns.rows[i].column_name] == "date") {
                                            newdata[columns.rows[i].column_name] = DatetoHTML(data[columns.rows[i].column_name]);
                                        }
                                        else {
                                            newdata[columns.rows[i].column_name] = data[columns.rows[i].column_name];
                                        }
                                    }
                                    return newdata;
                                })
                        ...
                        res.rows.forEach(row => {
                            Object.keys(row).forEach(key => {
                                if(row[key] instanceof Date){
                                    ...
                                    row[key] = new Date(row[key].getTime() - timezoneOffsetMinutes * 60 * 1000);
                                }
                            });
                        });
                        resolve(res);
                ...
                for (let i = 0; i < sorted.length; i++) {
                    ...
                    for (let j = 0; j < sorted[i].length; j++) {
                        for (let k = 0; k < Keys.length; k++) {
                            ...
                            if(false && Values[j * Keys.length + k] === null){ //Erstmal Ignorieren!
                                str = str + `$${j * Keys.length + k + 1}`;
                            }else{
                                str = str + `$${j * Keys.length + k + 1}::${DataTypes[Keys[k]]}`;
                            }
                    ...
                    str = str + "where ";
                    for (let j = 0; j < whereValues.length; j++) {
                        str = str + "c." + whereValues[j] + "::"+ DataTypes[whereValues[j]] +" = t." + whereValues[j];
    ...
    async InsertPG(array, table, toReturn = "") {
        ...
        return new Promise(function (resolve, reject) {
            ...
            let DataTypes;
            thisObject.Database.query({ text: `select * from information_schema.columns where table_name = '${table}'`, values: [] }, async (err, columns) => {
                let result = [];
                let tempRes;
                if (err) {
                    reject(err);
    ...
    async DeletePG(table, where = "", values = [], safety = true) {
        ...
        return new Promise(function (resolve, reject) {
            if (typeof where != "string") {
                reject("Error DeletePG: Input is not a String, type: " + typeof where + ".");
            }
            let query;
            if (where == "" && safety) {
                reject("Error DeletePG: you cannot delete your entire table with Saftey = true!");
    ...
    async getSchemaPG(table){
        let thisObject = this;
        return new Promise(function(resolve,reject){
            thisObject.Database.query({ text: `select * from information_schema.columns where table_name = '${table}'`, values: [] }, async function(err, columns) {
                if (err) {
                    reject(err)
                } else {
                            ...
                            try {
                                const res = await new Promise((resolve, reject) => {
                                    thisObject.Database.query({
                                        ...
                                        `,
                                        values: [table, column.column_name, column.udt_schema, column.udt_name],
                                    }, (error, res) => {
                                        if (error) {
                                            reject(error);
                                        } else {
